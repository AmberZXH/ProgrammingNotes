...
为前端（APP端、PC浏览器端、微信端）从后端服务器获取所需的数据提供一个简单并且统一的方式，不管是 APP 还是 WEB 浏览器，只要通过 HTTP 协议都可以实现同样的操作。
...
401	| Unauthorized | 请求没有权限的资源，要求用户的身份认证
...
HTTP 响应后端（服务器）返回给前端（客户端）的数据，由【响应行、响应头、响应体】三部分组成，主要用于返回前端的请求，操作的执行状态。返回数据，以及在前端接受到数据后如何解析。
